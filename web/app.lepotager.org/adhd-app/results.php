...
$trendQuestions = [
    ['primary_path', 'What people most want help with'],
    ['visual_home_density', 'Easiest home-screen style'],
    ['visual_focus_surface', 'Least distracting focus view'],
    ['visual_return_screen', 'Most comfortable way to return'],
    ['progress_style', 'Preferred long-term progress'],
    ['personality', 'Preferred app personality'],
    ['name_final', 'Name universe'],
    ['survey_save_helpful', 'Save & return'],
];

$trends = [];
if ($n >= 3) {
    foreach ($trendQuestions as [$qid, $title]) {
        $trend = analytics_public_trend($sessions, $qid, 2);
        if ($trend !== null) {
            $trends[] = ['title' => $title, 'trend' => $trend];
        }
    }
}
?><!doctype html>
...
<head>
  ...
  <title>What we’re learning so far — Le Potager Lab</title>
  <style>
:root {
  color-scheme: light;
  --bg: #f5f3ed;
...
<body>
<main class="page-card public-results">
  <p class="eyebrow">Le Potager Lab · ADHD app pre-test</p>
  <h1>What we’re learning so far 🌱</h1>
  <p class="lead">A small, privacy-first snapshot of early questionnaire results. This page updates automatically as more completed responses arrive.</p>

  <div class="summary-grid">
    <div class="summary-card"><small>Required responses submitted</small><strong><?= $n ?></strong></div>
    <div class="summary-card"><small>Research stage</small><strong>Early pre-test</strong></div>
  </div>

  <div class="notice">
    These results describe this small group of respondents only. They are not a diagnosis, a clinical finding or a representative estimate of people with ADHD.
  </div>

  <?php if ($n < 3): ?>
    <div class="theme-banner theme-green">
      <p><strong>It is still too early to publish response patterns safely.</strong> To protect participants in a very small group, breakdowns appear only after at least three required questionnaire responses have been submitted.</p>
    </div>
  <?php elseif (!$trends): ?>
    <div class="theme-banner theme-green">
      <p><strong>No repeated pattern is strong enough to publish yet.</strong> Public results hide answers selected by only one person, so a unique response cannot be singled out.</p>
    </div>
  <?php else: ?>
    <h2>Early repeated patterns</h2>
    <div class="analytics-grid">
      <?php foreach ($trends as $item): ?>
        <article class="analytics-card public-trend-card">
          <small><?= rh($item['title']) ?></small>
          <strong><?= rh(implode(' / ', $item['trend']['labels'])) ?></strong>
          <p>Selected by multiple participants.</p>
        </article>
      <?php endforeach; ?>
    </div>
  <?php endif; ?>
  ...
  <div class="privacy-panel">
    ...
    <p>No names, email addresses, response IDs, dates, times, demographics, diagnostic status or free-text comments appear here. Unique choices are suppressed rather than published.</p>
  </div>

  <div class="action-row">
    <a class="btn btn-primary" href="./">Take or resume the questionnaire</a>
    <a class="btn btn-secondary" href="https://www.lepotager.org/">Le Potager du Web</a>
  </div>
</main>
</body>
</html>

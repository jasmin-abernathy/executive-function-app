...
?>
/* Questionnaire application — CSP-safe */
/* v0.4.6: application embedded directly to bypass stalled static asset requests */
(() => {
  'use strict';
  ...
  async function init() {
    ...
    /*
     * Important:
     * Never wait for the server before the first render.
     * A stale/local resume token or a temporarily slow API must not produce
    ...
    if (token) {
      try {
        ...
        // resumeFromServer already falls back to the local copy.
      }
    }
  ...
  init().catch((error) => {
    console.error('[ADHD questionnaire init]', error);

    try {
      if (!state.current) state.current = 'welcome';
      render();
      setSaveStatus(
        'The server could not be reached. The saved copy on this device is still available.',
        'warning'
      );
    } catch (_) {
      const fallback = document.getElementById('survey-body');
      if (fallback) {
        fallback.innerHTML = `
          <div class="question-wrap">
            <p class="eyebrow">Temporary loading problem</p>
            <h1>The questionnaire could not initialise</h1>
            <p class="lead">You can start with a clean local session without deleting any submitted responses.</p>
            <div class="action-row">
              <a class="btn btn-primary" href="?fresh=1">Start a fresh local session</a>
              <a class="btn btn-secondary" href="results.php">View public results</a>
            </div>
          </div>`;
      }
    }

...
<head>
  ...
  <meta name="description" content="Privacy information for the Le Potager Lab ADHD app questionnaire.">
  <title>Privacy information — Le Potager Lab</title>
  <style>
:root {
  color-scheme: light;
  --bg: #f5f3ed;
...
<main class="page-card">
  ...
  <p class="lead">This page explains how information is handled in the adaptive questionnaire and the separate optional participation form.</p>

  <h2>Who is responsible?</h2>
  <p><strong><?= htmlspecialchars((string) $operator['legal_name'], ENT_QUOTES, 'UTF-8') ?></strong>, operating as <?= htmlspecialchars((string) $operator['name'], ENT_QUOTES, 'UTF-8') ?>.</p>
  <p><?= nl2br(htmlspecialchars((string) $operator['postal_address'], ENT_QUOTES, 'UTF-8')) ?><br>
  Contact: <a href="mailto:<?= htmlspecialchars((string) $operator['contact_email'], ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars((string) $operator['contact_email'], ENT_QUOTES, 'UTF-8') ?></a></p>

  <h2>What is this project?</h2>
  <p>This is participatory product research used to understand needs, preferences and interface choices for a possible ADHD-friendly application. It is not a diagnostic questionnaire, a treatment, or a clinical research study. The questionnaire itself is restricted to participants aged 18 or older in this pre-test. A separate email-only notification form may be used by people who cannot take part in the current age-restricted round and only want to hear if a future age-appropriate research process opens.</p>

  <h2>Questionnaire responses</h2>
  <p>The questionnaire may contain optional information about your relationship with ADHD and the difficulties you experience. This may be sensitive information. It is processed only after your explicit agreement.</p>
  <p>Questionnaire answers are stored under a random identifier. No name or email address is required to submit them.</p>

  <h2>Save and resume information</h2>
  <p>If you request an emailed resume link or reminder, your email address is encrypted and stored in a separate table. It is used only for that resume link and the one reminder you select. It does not subscribe you to project updates.</p>

  <h2>Optional participation and future-research contact forms</h2>
  <p>After finishing the questionnaire, you may separately choose to join research, co-design, beta testing, accessibility testing, open-source contribution, translation, professional advisory or project-update lists. Nothing is preselected.</p>
  <p>People who cannot take part in the current 18+ pre-test may also use a separate email-only form to request a notification if a future age-appropriate research round becomes available. That form does not request a name, date of birth, exact age, ADHD information or questionnaire answers.</p>
  <p>Contact preferences become active only after you confirm the link sent to your email address. Participation contacts are never directly linked to questionnaire responses.</p>

  <h2>Purposes and legal basis</h2>
  <ul>
    <li><strong>Questionnaire:</strong> analysing product needs and design preferences, based on your consent and explicit consent for optional sensitive information.</li>
    <li><strong>Resume link and reminder:</strong> providing the service you specifically request.</li>
    <li><strong>Participation messages:</strong> contacting you about the categories you select, based on your consent.</li>
    <li><strong>Future age-appropriate research notification:</strong> sending a message only if such a research round becomes available, based on your consent.</li>
    <li><strong>Security and maintenance:</strong> protecting the service and diagnosing technical failures, based on legitimate interests.</li>
  </ul>

  <h2>Retention choices</h2>
  <ul>
    <li>Unfinished questionnaire sessions: automatically deleted after <?= (int) $retention['unfinished_days'] ?> days without activity.</li>
    <li>Completed questionnaire responses: retained for up to <?= (int) $retention['completed_months'] ?> months, then deleted or irreversibly aggregated.</li>
    <li>Resume/reminder contact data: deleted when the questionnaire is submitted, or no later than <?= (int) $retention['reminder_days_after_send'] ?> days after a reminder is sent.</li>
    <li>Unconfirmed participation requests: deleted after 30 days.</li>
    <li>Confirmed participation details: kept until you unsubscribe or request deletion, subject to necessary evidence of consent.</li>
  </ul>

  <h2>Recipients and hosting</h2>
  <p>Data is intended for the project owner and authorised project collaborators who need it for research or project organisation. The site is designed without advertising trackers or third-party analytics. Hosting and email providers may process technical data as service providers.</p>

  <h2>Your choices and rights</h2>
  <p>You may pause, resume or delete an unfinished questionnaire. You may unsubscribe from project messages using the link provided in those emails.</p>
  <p>You may request access, correction, deletion, restriction, portability where applicable, or withdrawal of consent by contacting <a href="mailto:<?= htmlspecialchars((string) $operator['contact_email'], ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars((string) $operator['contact_email'], ENT_QUOTES, 'UTF-8') ?></a>. Withdrawing consent does not affect processing carried out before withdrawal.</p>

  <h2>Cookies and local storage</h2>
  <p>The questionnaire uses browser local storage to keep your progress on the same device and to store the private random resume token. It does not use this storage for advertising or cross-site tracking. You can remove it using the “Delete my saved questionnaire progress” control or your browser settings.</p>

  <h2>Security</h2>
  <p>Database access uses prepared queries. Email addresses and names are encrypted at application level. Resume, confirmation and unsubscribe links use high-entropy private tokens stored as cryptographic hashes where possible.</p>

  <div class="action-row"><a class="btn btn-primary" href="<?= htmlspecialchars(rtrim((string) config('site_url'), '/') . '/', ENT_QUOTES, 'UTF-8') ?>">Return to the questionnaire</a></div>
</main>
</body>
</html>

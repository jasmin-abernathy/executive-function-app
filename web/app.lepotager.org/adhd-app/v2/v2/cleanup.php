...
// Remove stale one-shot reminder records. Successful reminders are deleted
// immediately by the shared reminder cron; this catches abandoned/failed rows.
try {
    v2_ensure_resume_reminders_table($pdo);
    $pdo->exec(
        "DELETE FROM survey_v2_resume_reminders
         WHERE reminder_at < DATE_SUB(NOW(), INTERVAL 7 DAY)"
    );
} catch (Throwable $reminderError) {
    error_log('[ADHD survey V2 cleanup reminder] ' . $reminderError->getMessage());
}

...
<head>
  ...
  <meta name="description" content="Optional email notification for a future age-appropriate Le Potager Lab ADHD app research round.">
  <meta name="robots" content="noindex,follow">
  <title>Future age-appropriate research — Le Potager Lab</title>
  <style>
:root {
  color-scheme: light;
  --bg: #f5f3ed;
...
<body>
<a class="skip-link" href="#future-research-main">Skip to the form</a>

<div class="site-shell">
  <header class="site-header">
    <a class="brand" href="<?= htmlspecialchars(rtrim((string) config('site_url'), '/') . '/', ENT_QUOTES, 'UTF-8') ?>">
      <span class="brand-mark" aria-hidden="true">🌱</span>
      <span>
        <strong>Le Potager Lab</strong>
        <span>Participatory product research</span>
      </span>
    </a>
  </header>

  <main id="future-research-main" class="survey-frame standalone-future-page" tabindex="-1">
    <div class="question-wrap">
      <p class="eyebrow">Future research</p>
      <h1>Interested in a future age-appropriate research round?</h1>

      <p class="lead">
        The current questionnaire pre-test is limited to adults aged 18 or older.
        If you cannot take part in this round, you may leave an email address here
        and we will contact you only if a future research round with an age-appropriate
        process becomes available.
      </p>

      <section class="future-youth-card">
        <h2>Only an email address is needed</h2>

        <div class="privacy-mini-grid" aria-label="What this form does and does not store">
          <div class="privacy-mini-item">
            <strong>✓ Stored</strong>
            <span>Your encrypted email address and this notification preference.</span>
          </div>
          <div class="privacy-mini-item">
            <strong>✕ Not requested here</strong>
            <span>Your name, date of birth, exact age, ADHD information or questionnaire answers.</span>
          </div>
          <div class="privacy-mini-item">
            <strong>↔ Separate</strong>
            <span>This form does not create or reconnect a questionnaire response.</span>
          </div>
          <div class="privacy-mini-item">
            <strong>✉ Confirmation required</strong>
            <span>The preference becomes active only after you confirm the link sent by email.</span>
          </div>
        </div>

        <form id="future-research-form" novalidate>
          <label class="field-label" for="future-research-email">Email address</label>
          <input
            class="text-field"
            id="future-research-email"
            name="email"
            type="email"
            inputmode="email"
            autocomplete="email"
          ...
          <label class="consent-row future-youth-consent">
            <input type="checkbox" id="future-research-consent" required>
            <span>
              I want to receive an email only if a future age-appropriate research round becomes available.
              I understand that I can unsubscribe at any time.
            </span>
          </label>

          <div id="future-research-feedback" aria-live="polite"></div>

          <div class="action-row">
            <button class="btn btn-primary btn-large" id="future-research-submit" type="submit">
              Notify me about a future round
            </button>
            <a class="btn btn-secondary" href="<?= htmlspecialchars(rtrim((string) config('site_url'), '/') . '/', ENT_QUOTES, 'UTF-8') ?>">
        ...
        <p class="small-text">
          If you do not confirm the email, the request remains inactive and is automatically removed
          under the project’s retention rules.
        </p>
      </section>

      <div class="notice">
        <strong>This is not a waiting list for diagnosis or care.</strong>
        It is only a notification preference for future product-research participation.
      </div>
    </div>
  </main>

  <footer class="small-text" style="padding:18px 5px 0;display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap">
    <span>No advertising trackers or third-party analytics.</span>
    <span>
      <a href="privacy.php">Privacy information</a> ·
      <a href="https://www.lepotager.org/">Le Potager du Web</a>
    </span>
  </footer>
</div>
...
(() => {
  ...
  const form = document.getElementById('future-research-form');
  const emailField = document.getElementById('future-research-email');
  const consentField = document.getElementById('future-research-consent');
  const feedback = document.getElementById('future-research-feedback');
  const submitButton = document.getElementById('future-research-submit');
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (!email || !emailField.checkValidity()) {
      feedback.innerHTML = '<div class="notice is-error">Enter a valid email address.</div>';
      emailField.focus();
      return;
    }

    if (!consentField.checked) {
      feedback.innerHTML = '<div class="notice is-error">Confirm that you want this future-round notification.</div>';
      consentField.focus();
      return;
    }
    ...
    try {
      const response = await fetch('api.php?action=participate', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok || !data.ok) {
        throw new Error(data.error || 'The server could not save this request.');
      }
      ...
      feedback.innerHTML =
        ...
        'No questionnaire response is attached to this request.' +
        '</div>';

      submitButton.textContent = 'Confirmation email sent';
    } catch (error) {
      feedback.innerHTML =
        '<div class="notice is-error">' +
        escapeHtml(error.message || 'A technical error occurred. Please try again.') +
        '</div>';
